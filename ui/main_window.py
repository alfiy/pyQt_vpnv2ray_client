...
2. 用 icon_helper.load_window_icon() 多路径探测窗口图标，Ubuntu 标题栏图标不显示
   时自动使用程序化生成的盾牌图标兜底
3. 按钮状态统一由 _refresh_buttons() 管理，根据 vpn_pid / v2ray_pid 真实值计算，
   消除各处手动 setEnabled 导致的状态不一致问题
"""
import os
import sys
import time
...
from core.icon_helper import (
    ...
    load_window_icon, apply_window_icon, force_titlebar_icon_x11, emoji_supported,
)
...
class MainWindow(QMainWindow):
    ...
    def _refresh_buttons(self, starting=False):
        """
        根据当前运行状态统一刷新所有按钮的启用/禁用。
        starting=True 表示正在启动中，所有启动按钮临时禁用。

        规则：
          启动按钮 → 对应服务已在运行 或 正在启动中 时禁用
          停止按钮 → 对应服务未在运行时禁用
          一键启动 → 两个服务都在运行 或 正在启动中 时禁用
          一键停止 → 两个服务都未运行时禁用
        """
    ...
    def start_combined(self):
        ...
        if tproxy_enabled:
            ...
            if not vps_ip or not self._validate_ip(vps_ip):
                ...
                return
        save_tproxy_config(
            tproxy_enabled, self.vps_ip_input.text().strip(),
            self.tproxy_port_input.value(),
            self.mark_input.value(), self.table_input.value())
        self._refresh_buttons(starting=True)
